...
)
{
    ...
    x_tmp[0] = *((uint64_t *)&scalar[0]);
    x_tmp[1] = *((uint64_t *)&scalar[8]);
    x_tmp[2] = *((uint64_t *)&scalar[16]);
    x_tmp[3] = *((uint64_t *)&scalar[24]);

    felem_contract(x,x_tmp);
}

void
traits::felem_to_scalar(
    scalar_type&        out,    // (O) Scalar (little endian)
    felem_type const&   in      // (I) Field element
)
{
    *((uint64_t *)&out[0]) = in[0];
    *((uint64_t *)&out[8]) = in[1];
 ...
 */
int
traits::jacobian_to_affine(
    felem_type          x_out,  // (O) affine x coordinate
    felem_type          y_out,  // (O) affine y coordinate
    felem_type const&   x_in_,  // (I) X coordinate
    felem_type const&   y_in_,  // (I) Y coordinate
    felem_type const&   z_in_   // (I) Z coordinate
)
{
    // Return error if point at infinity
    if (smallfelem_is_zero(z_in_))
    {
        return -1;
    }
    ...
    // Convert to long limb type
    smallfelem_expand(x_in, x_in_);
    smallfelem_expand(y_in, y_in_);
    smallfelem_expand(z1, z_in_);
...
/* Calculate scalar multiple of the curve base point
 */
void
...
)
{
    ...
    for (i = 31; i >= 0; --i) {
        ...
        select_point(bits, 16, gmul[1], tmp);
        ...
        /* select the point to add, in constant time */
        select_point(bits, 16, gmul[0], tmp);
        /* Arg 1 below is for "mixed" */
        point_add(nq[0], nq[1], nq[2],
                  nq[0], nq[1], nq[2], 1, tmp[0], tmp[1], tmp[2]);
    }
    felem_contract(x_out, nq[0]);
    felem_contract(y_out, nq[1]);
    felem_contract(z_out, nq[2]);
}

/*
 * Scalar multiplication with respect to an arbitrary point.
 * 
 * The small point multiples (0P,...,16P) are stored in pre_comp
 *
 * Output point (X, Y, Z) is stored in x_out, y_out, z_out and are minimally reduced
 */
void
...
)
{
    ...
    for (i = 255; i >= 0; --i) {
        ...
        if ((i % 5 == 0)) {
            ...
            bits = get_bit(scalar, i + 4) << 5;
            bits |= get_bit(scalar, i + 3) << 4;
            bits |= get_bit(scalar, i + 2) << 3;
            bits |= get_bit(scalar, i + 1) << 2;
            bits |= get_bit(scalar, i) << 1;
            bits |= get_bit(scalar, i - 1);
            ecp_utils::ec_GFp_nistp_recode_scalar_bits(&sign, &digit, bits);

            /*
             * select the point to add or subtract, in constant time
             */
            select_point(digit, 17, pre_comp, tmp);
...
traits::precompute_table(
    precomp_table           pre_comp,   // (O) Precompute Table
...
{
    smallfelem_zero(pre_comp[0][0]);
    smallfelem_zero(pre_comp[0][1]);
    smallfelem_zero(pre_comp[0][2]);

    smallfelem_assign(pre_comp[1][0],X);
    smallfelem_assign(pre_comp[1][1],Y);
    smallfelem_assign(pre_comp[1][2],Z);

    for (int j = 2; j <= 16; ++j) {
        if (j & 1) {
            point_add_small(pre_comp[j][0], pre_comp[j][1],
                            pre_comp[j][2], pre_comp[1][0],
                            pre_comp[1][1], pre_comp[1][2],
                            pre_comp[j - 1][0],
                            pre_comp[j - 1][1],
                            pre_comp[j - 1][2]);
        } else {
            point_double_small(pre_comp[j][0],
                               pre_comp[j][1],
                               pre_comp[j][2],
                               pre_comp[j / 2][0],
                               pre_comp[j / 2][1],
...
void felem_small_sum(felem out, const smallfelem in)
{
    out[0] += in[0];
    out[1] += in[1];
    out[2] += in[2];
...
void felem_scalar(felem out, const u64 scalar)
{
    out[0] *= scalar;
    out[1] *= scalar;
    out[2] *= scalar;
...
void longfelem_scalar(longfelem out, const u64 scalar)
{
    out[0] *= scalar;
    out[1] *= scalar;
    out[2] *= scalar;
...
int smallfelem_is_zero_int(const smallfelem small)
{
...
char get_bit(const felem_bytearray in, int i)
{
    if ((i < 0) || (i >= 256))
        return 0;
    return (in[i >> 3] >> (i & 7)) & 1;
